namespace Frida {
	...
	public sealed class LinuxHostSession : LocalHostSession {
		...
		construct {
			...
			agent = new AgentDescriptor (PathTemplate ("fs-agent-<arch>.so"),
				new Bytes.static (blob32.data),
				new Bytes.static (blob64.data),
				new AgentResource[] {
					new AgentResource ("fs-agent-arm.so", new Bytes.static (emulated_arm.data), tempdir),
					new AgentResource ("fs-agent-arm64.so", new Bytes.static (emulated_arm64.data), tempdir),
				},
				AgentMode.INSTANCED,
				tempdir);
#endif
		...
		protected override string? get_emulated_agent_path (uint pid) throws Error {
			...
			switch (cpu_type_from_pid (pid)) {
				case Gum.CpuType.IA32:
					name = "fs-agent-arm.so";
					break;
				case Gum.CpuType.AMD64:
					name = "fs-agent-arm64.so";
					break;
				default:
